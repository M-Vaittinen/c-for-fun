...
if (isset($_POST['keepid'])) {
	debug_print("total POST cards ".count($_POST['keepid']));
	if (isset($_POST['rate'])) {
...
if ($DBG && $preselected) {
	if ($preselected[0]) {
		debug_print("Cheap:" .count($preselected[0]));
		foreach($preselected[0] AS $cheap)
			debug_print($cheap.",");
	}

	if ($preselected[1]) {
		debug_print("Mid:".count($preselected[1]));
		foreach($preselected[1] AS $mid)
			debug_print($mid.",");
	}

	if ($preselected[2]) {
		debug_print("Expencive:".count($preselected[2]));
		foreach($preselected[2] AS $e)
			debug_print($e.",");
	}
}
...
foreach($PRIZEBUCKETS as $PRIZE_LIMIT) {
	...
	debug_print("preselected $num_presel, num to rand $num_cards_to_rand");
	debug_print("Add ".count($selected)." cards to ".$card_group_names[$i]);
